...
namespace Cervel.TimeParser
{
    public class TimeExpressionV2Listener : TimeExpressionV2BaseListener
    {
        ...
        public override void EnterYearsUntil(TimeExpressionV2Parser.YearsUntilContext context)
        {
            OpenScope(nameof(EnterYearsUntil));
        }

        public override void ExitYearsUntil(TimeExpressionV2Parser.YearsUntilContext context)
        {
            var years = _scope.YearGenerators.Consume();

            CloseScope();

            if (years.Length > 1)
                _scope.YearGenerators.Add(Time.Until(years[1], years[0]));
            else
                _scope.YearGenerators.Add(years[0]);
        }


        public override void EnterYearsSince(TimeExpressionV2Parser.YearsSinceContext context)
        {
            OpenScope(nameof(EnterYearsSince));
        }

        public override void ExitYearsSince(TimeExpressionV2Parser.YearsSinceContext context)
        {
            var years = _scope.YearGenerators.Consume();
            ...
            if (years.Length > 1)
                _scope.YearGenerators.Add(Time.Since(years[1], years[0]));
            else
                _scope.YearGenerators.Add(years[0]);
        }


        public override void EnterYearsExcept(TimeExpressionV2Parser.YearsExceptContext context)
        {
            OpenScope(nameof(EnterYearsExcept));
        }

        public override void ExitYearsExcept(TimeExpressionV2Parser.YearsExceptContext context)
        {
            var years = _scope.YearGenerators.Consume();
            ...
            if (years.Length > 1)
                _scope.YearGenerators.Add(Time.Outside(years[1], years[0]));
            else
                _scope.YearGenerators.Add(years[0]);
        }


        public override void EnterYearsNEveryM(TimeExpressionV2Parser.YearsNEveryMContext context)
        {
            OpenScope(nameof(EnterYearsNEveryM));
        }

        public override void ExitYearsNEveryM(TimeExpressionV2Parser.YearsNEveryMContext context)
        {
            var years = _scope.YearGenerators.ConsumeSingle();
            if (_scope.Numbers.HasValues())
            {
                var numbers = _scope.Numbers.Consume();
                years = years.NEveryM(numbers[0], numbers[1]);
            }

            CloseScope();

            _scope.YearGenerators.Add(years);
        }
